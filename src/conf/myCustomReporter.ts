...
export default class CustomReporter implements Reporter {
    ...
    async onEnd(result: FullResult): Promise<void> {
        ...
        // === Determine where to send report ===
        const reportChannels = process.env.REPORT_CHANNELS?.split(",").map(ch => ch.trim().toLowerCase()) || [];

        // --- Slack ---
        if (reportChannels.includes("slack")) {
            const bodySlack = {
        ...
        // --- Discord ---
        if (reportChannels.includes("discord")) {
            const bodyDiscord = {
        ...
        // --- Telegram ---
        if (reportChannels.includes("telegram")) {
